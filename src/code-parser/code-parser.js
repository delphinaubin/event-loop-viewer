...
const defaultStrategy = {
    isApplicable: () => true,
    getCode: (node) => generate(node).code
}

const setTimeoutStrategy = {
    isApplicable: (node) => (
    ...
    getCode: (node) => {
        ...
        return removeAddedLineFeed(generate(node).code)
    }
}

const fsReadFileStrategy = {
    isApplicable: (node) => (
        node.type === 'ExpressionStatement'
        && node.expression.type === 'CallExpression'
...
function removeAddedLineFeed(code) {
    return code
        .replace('\\nif', 'if')
        .replace('\\nwhile', 'while')
}
